...
class CumulativeWindowRefinement:
    ...
    def update(self, text, language, final, protector, confidence):
        with self.lock:
            ...
            for chunk in chunks[shared:start]:
                result = self.refine(chunk, language, False, protector, confidence,
                                     single_window=True)
            ...
            if self.active is None or self.active[0] != source:
                result = self.refine(source, language, False, protector, confidence,
                                     single_window=True)
